...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		List<Integer> toCheck = getNeighbors(v);
		ArrayList<Integer> list = new ArrayList<>();
		for (int i : toCheck) {
			list.addAll(getNeighbors(i));
		}

		return list;
	}
	
	public List<Integer> getDistance2Matrix(int v) {
//		CORRECTED ATTEMPT at matrix multiplication
		int[][] newMatrix = new int[adjMatrix.length][adjMatrix.length];
		for (int i = 0; i < adjMatrix.length; i++) {
			for (int j = 0; j < adjMatrix.length; j++) {
				for (int k = 0; k < adjMatrix.length; k++) {
					newMatrix[i][j] += (adjMatrix[i][k] * adjMatrix[k][j]);
				}
			}
		}
		ArrayList<Integer> list = new ArrayList<>();
